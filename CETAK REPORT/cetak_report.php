<?php
// =======================================================
// SOLUSI: Tambahkan session_start() di paling atas
// =======================================================
session_start();
...
// Query utama untuk mengambil data AFRN (sudah benar, tidak diubah)
$query = "SELECT
    a.id_afrn, a.no_afrn, a.tgl_afrn, a.no_bpp, a.id_transportir, a.id_destinasi, a.id_tangki, a.rit,
    a.dibuat, a.diperiksa, a.disetujui, a.id_driver, 
...
LEFT JOIN DESTINASI d ON a.id_destinasi = d.id_destinasi
LEFT JOIN TANGKI t ON a.id_tangki = t.id_tangki
LEFT JOIN DRIVER c ON a.id_driver = c.id_driver
LEFT JOIN BON bo ON a.id_bon = bo.id_bon
...
// =========================================================================
// PENGAMBILAN NAMA LENGKAP USER LOGIN (QUALITY CONTROLLER)
// Blok kode yang dobel sudah dibersihkan dan hanya ada satu blok ini.
// =========================================================================
$id_user_login = $_SESSION['id_user'] ?? 0;
$nama_lengkap_qc = 'Petugas Tidak Dikenal'; // Nilai default

if ($id_user_login > 0) {
    $stmt_user = $conn->prepare("SELECT nama_lengkap FROM user WHERE id_user = ?");
    if ($stmt_user) {
        $stmt_user->bind_param("i", $id_user_login);
        $stmt_user->execute();
        $result_user = $stmt_user->get_result();
        if ($result_user->num_rows > 0) {
            // Ini adalah variabel yang akan kita gunakan untuk Quality Controller
            $nama_lengkap_qc = $result_user->fetch_assoc()['nama_lengkap'];
        }
        $stmt_user->close();
    }
}
...
function render_afrn($data, $nama_lengkap_qc) { // Variabel baru ditambahkan sebagai parameter?>
<div class="afrn-copy bg-white">
    ...
    <div class="main-content-tables flex h-auto">
            ...
            <table class="w-full border-collapse">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="table-cell"><?php echo htmlspecialchars($nama_lengkap_qc); ?></td>
                    </tr>
                    ...
                    <tr>
                        <td class="table-cell">Quality Controller</td>
                        <td class="table-cell"><?php echo htmlspecialchars($data['diperiksa'] ?? ''); ?></td>
                    </tr>
                    <tr>
        ...
        <div class="right-column">
            ...
            <div class="section-box text-xxs-force mb-4 border border-gray-900 p-3 ">
            ...
            <div class="p-1 w-full text-center">
                ...
                <div class="text-xs font-semibold mb-6"><?php echo htmlspecialchars($nama_lengkap_qc); ?></div>
...
<body>
    <div class="print-container">
        <?php render_afrn($data, $nama_lengkap_qc); ?>
    </div>
</body>
